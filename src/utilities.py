...
This module contains reusable PySpark utility functions commonly used in data pipelines,
...
def remove_duplicates(df, subset_cols=None):
    ...
    :param subset_cols: List of columns to check for duplicates. If None, uses all columns.
    :return: Deduplicated DataFrame
    """
    return df.drop_duplicates(subset=subset_cols)

# Further enhancement scope: add functionality to provide ordering columns and keep first or last (use row number window function)
...
def fill_nulls(df, fill_dict):
    ...
    :param df: Input DataFrame
    :param fill_map: Dictionary with column names as keys and fill values as values.
